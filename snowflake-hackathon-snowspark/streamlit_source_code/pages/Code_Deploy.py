...
# def deploy_code(obj_type,src_db,src_schma,src_obj,dest_db,dest_schma):
#     # st.session_state['final']=''
#     query='''call db_test.sc_test.SP_GET_DDL('{obj_type}','{src_db}','{src_schma}','{src_obj}','{dest_db}','{dest_schma}')
#     '''.format(obj_type=obj_type,src_db=src_db,src_schma=src_schma,src_obj=src_obj,dest_db=dest_db,dest_schma=dest_schma)
#     res=st.session_state['sf_con'].execute(query)
#     # st.write(res.fetchall()[0][0])
    # st.session_state['final']=res.fetchall()[0][0]
#     final=res.fetchall()[0][0]
...
if 'connect' not in st.session_state:
    ...
    if st.session_state['connect']==True:
        st.markdown('''<h2 style='color:#008080;font-family: Rockwell;text-align: center;'>Code Deployment...</h2>''',unsafe_allow_html=True)

        col1,col2=st.columns(2)
        with col1:
            st.write('Source')
            st.session_state['src_db']=st.selectbox('Enter Source Database Name',get_db_list(),key=0)
            st.session_state['src_sc']=st.selectbox('Enter Source Schema Name',get_schema_list(st.session_state['src_db']),key=1)
            st.session_state['obj_type']=st.selectbox('Enter Object Type',['TABLE','PROCEDURE'])
            if st.session_state['obj_type']=='TABLE':
                st.session_state['src_obj']=st.selectbox('Enter Source Table Name',get_table_list(st.session_state['src_db'],st.session_state['src_sc']))
            else:
                st.session_state['src_obj']=st.selectbox('Enter Source Procedure Name',get_procedure_list(st.session_state['src_db'],st.session_state['src_sc']))
        with col2:
            st.write('Destination')
            st.session_state['des_db']=st.selectbox('Enter Destination Database Name',get_db_list())
            st.session_state['des_sc']=st.selectbox('Enter Destination Schema Name',get_schema_list(st.session_state['des_db']))
        ...
        # st.button('Deploy',on_click=deploy_code,args=[st.session_state['obj_type'],st.session_state['src_db'],st.session_state['src_sc'],st.session_state['src_obj'],st.session_state['des_db'],st.session_state['des_sc']])
        if st.button('Deploy'):
            query='''call db_test.sc_test.SP_GET_DDL('{obj_type}','{src_db}','{src_schma}','{src_obj}','{dest_db}','{dest_schma}')
            '''.format(obj_type=st.session_state['obj_type'],src_db=st.session_state['src_db'],src_schma=st.session_state['src_sc'],src_obj=st.session_state['src_obj'],dest_db=st.session_state['des_db'],dest_schma=st.session_state['des_sc'])
            res=st.session_state['sf_con'].execute(query)
            st.write(res.fetchall()[0][0])
    else:
