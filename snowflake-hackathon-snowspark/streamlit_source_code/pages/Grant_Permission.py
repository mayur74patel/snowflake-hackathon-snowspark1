import streamlit as st
import snowflake.connector as sc
from io import StringIO

st.set_page_config(page_title='Hackathon',page_icon=':snowflake:')
st.markdown('<style>.block-container{border-style: solid;border-width:3px;margin-top:50px;border-image: linear-gradient(120deg, #c71585 25%, grey 25%, #4169E1 50%,red 50%, #191970 75%, teal 75%) 5;}\
    .block-container.css-128j0gw{visibility: hidden;}p{font-weight:1000;color:#000000;} .css-1offfwp{font-color:black}\
       footer {visibility: hidden;}</style>',unsafe_allow_html=True)
       
with open('style.css')as f:
    st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html = True)

st.markdown(f""" <style> div.stButton > button:first-child {{  appearance: none;   background-color: transparent;   border: 2px solid #1A1A1A;   border-radius: 15px;   box-sizing: border-box;   color: #3B3B3B;   cursor: pointer;   display: inline-block;   font-family: Roobert,-apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif,"Apple Color Emoji","Segoe UI Emoji","Segoe UI Symbol";   font-size: 16px;   font-weight: 600;   line-height: normal;   margin: 0;   min-height: 60px;   min-width: 0;   outline: none;   padding: 16px 24px;   text-align: center;   text-decoration: none;   transition: all 300ms cubic-bezier(.23, 1, 0.32, 1);   user-select: none;   -webkit-user-select: none;   touch-action: manipulation;  will-change: transform;}} <style> """ , unsafe_allow_html=True)

def get_user_list():
    query="SELECT distinct NAME from SNOWFLAKE.ACCOUNT_USAGE.USERS where Name not in ('SNOWFLAKE')  and DELETED_ON is null order by 1 "
    res=st.session_state['sf_con'].execute(query) 

    lst=[]
    for row in res.fetchall():  
        for r in row:
            lst.append(r)
    return lst

def get_role_list():
    query='''show roles'''
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[1])
    return lst

def get_db_list():
    query="SELECT distinct DATABASE_NAME from information_schema.databases \
        where DATABASE_NAME!='SNOWFLAKE_SAMPLE_DATA order by 1'"
    res=st.session_state['sf_con'].execute(query) 

    lst=[]
    for row in res.fetchall():  
        for r in row:
            lst.append(r)
    return lst
 
def get_schema_list(db):
    query="SELECT distinct schema_name from  "+db+".INFORMATION_SCHEMA.SCHEMATA \
        where SCHEMA_NAME!='INFORMATION_SCHEMA' and CATALOG_NAME='"+db+"' order by 1"
    res=st.session_state['sf_con'].execute(query) 

    lst=[]
    for row in res.fetchall():  
        for r in row:
            lst.append(r)
    return lst

def get_table_list(db,schema):
    query="SELECT distinct table_name from  "+db+".INFORMATION_SCHEMA.TABLES \
        where TABLE_SCHEMA!='INFORMATION_SCHEMA' and TABLE_SCHEMA='"+schema+"' order by 1"
    res=st.session_state['sf_con'].execute(query) 

    lst=[]
    for row in res.fetchall():  
        for r in row:
            lst.append(r)
    return lst

def get_table_column_list(db,schema,table):
    if table is not None:
        query="SELECT distinct COLUMN_NAME from  "+db+".INFORMATION_SCHEMA.COLUMNS \
        where TABLE_SCHEMA!='INFORMATION_SCHEMA' and TABLE_SCHEMA='"+schema+"'and TABLE_name='"+table+"' order by 1"
        res=st.session_state['sf_con'].execute(query) 

        lst=[]
        for row in res.fetchall():  
            for r in row:
                lst.append(r)
        return lst
    else:
        return[]


def get_wh_list():
    # query='''SELECT DISTINCT WAREHOUSE_NAME FROM SNOWFLAKE.ACCOUNT_USAGE.WAREHOUSE_METERING_HISTORY
    # WHERE WAREHOUSE_NAME NOT IN ('CLOUD_SERVICES_ONLY') order by 1'''
    query='''show warehouses'''
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[0])
    return lst

def give_query(db,role):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    privi=['SCHEMA','EXTERNAL TABLE','FILE FORMAT','FUNCTION','MATERIALIZED VIEW','PROCEDURE','SEQUENCE','STAGE','STREAM','TABLE','TASK','VIEW']
    for p in privi:
        query='''grant all on all {obj}S in database {db} to role {role}'''.format(obj=p,db=db,role=role)
        res=st.session_state['sf_con'].execute(query)
    st.write('Statement executed successfully. ')

def give_access_user(user,role):
    query='''grant role {role} to user {user} '''.format(role=role,user=user)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_schema(db,role,schema):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    privi=['EXTERNAL TABLE','FILE FORMAT','FUNCTION','MATERIALIZED VIEW','PROCEDURE','SEQUENCE','STAGE','STREAM','TABLE','TASK','VIEW']
    for p in privi:
        query='''grant all on all {obj}S in schema {db}.{schema} to role {role}'''.format(obj=p,db=db,schema=schema,role=role)
        res=st.session_state['sf_con'].execute(query)
        #st.write(res.fetchall())
    st.write('Statement executed successfully. ')

def give_access_table(db,role,schema,table):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on  {db}.{schema}.{table} to role {role}'''.format(db=db,schema=schema,table=table,role=role)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_view(db,role,schema,view):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on  {db}.{schema}.{view} to role {role}'''.format(db=db,schema=schema,view=view,role=role)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_procedure(db,role,schema,procedure):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant usage on procedure {db}.{schema}.{procedure} to role {role}'''.format(db=db,schema=schema,procedure=procedure,role=role)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_stages(db,role,schema,stage):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on stage  {db}.{schema}.{stage} to role {role}'''.format(db=db,schema=schema,stage=stage,role=role)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_functions(db,role,schema,function):
    query='''grant all on database {db} to role {role}'''.format(db=db,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant all on schema {db}.{schema} to role {role}'''.format(db=db,schema=schema,role=role)
    res=st.session_state['sf_con'].execute(query)
    query='''grant usage on function  {db}.{schema}.{function} to role {role}'''.format(db=db,schema=schema,function=function,role=role)
    res=st.session_state['sf_con'].execute(query)
    # st.write(res.fetchall()[0][0])
    st.write('Statement executed successfully. ')

def give_access_wh(wh,role):
    query='''grant all on warehouse {wh} to role {role}'''.format(wh=wh,role=role)
    res=st.session_state['sf_con'].execute(query)
    # test=res.fetchall()
    # st.write(test[0][0])
    st.write('Statement executed successfully. ')

def get_avialble_type_schema(db,schema):
    query='''select DISTINCT type from (
            select 'TABLES' TYPE,COUNT(*) CNT FROM {db}.INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and TABLE_SCHEMA='{schema}' 
            UNION
            select 'VIEWS' TYPE,COUNT(*) CNT FROM {db}.INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and TABLE_SCHEMA='{schema}'
            UNION
            select 'STAGES' TYPE,COUNT(*) CNT FROM {db}.INFORMATION_SCHEMA.STAGES WHERE STAGE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and STAGE_SCHEMA='{schema}'
            ) where cnt!=0 ORDER BY 1
            '''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[0])
    return lst

def get_list_tables(db,schema):
    query='''select distinct table_name FROM {db}.INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and TABLE_SCHEMA='{schema}'
            order by 1'''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[0])
    return lst

def get_list_functions(db,schema):
    query='''select distinct FUNCTION_NAME,ARGUMENT_SIGNATURE FROM {db}.INFORMATION_SCHEMA.FUNCTIONS \
        WHERE FUNCTION_SCHEMA NOT IN ('INFORMATION_SCHEMA') and FUNCTION_SCHEMA='{schema}' order by 1
            '''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1:
        if row[1]!='()':
            temp=row[1].replace('(','').replace(')','')
            test=''
            if len(temp.split(','))>1:
                for t in temp.split(','):
                    test=test+t.strip().split(' ')[1]+','
                lst.append(row[0]+'('+test[:-1]+')')
            else:
                temp=temp.split(' ')[1]
                proc_name=row[0]+'('+temp+')'
                lst.append(proc_name)
        else:
            proc_name=row[0]+row[1]
            lst.append(proc_name)
    return lst

def get_list_procedures(db,schema):
    query='''select distinct procedure_name,ARGUMENT_SIGNATURE,CONCAT(procedure_name,ARGUMENT_SIGNATURE) AS NAME FROM {db}.INFORMATION_SCHEMA.PROCEDURES \
        WHERE PROCEDURE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and PROCEDURE_SCHEMA='{schema}' ORDER BY NAME
            '''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1:
        if row[1]!='()':
            temp=row[1].replace('(','').replace(')','')
            test=''
            if len(temp.split(','))>1:
                for t in temp.split(','):
                    test=test+t.strip().split(' ')[1]+','
                lst.append(row[0]+'('+test[:-1]+')')
            else:
                temp=temp.split(' ')[1]
                proc_name=row[0]+'('+temp+')'
                lst.append(proc_name)
        else:
            proc_name=row[0]+row[1]
            lst.append(proc_name)

    return lst

def get_list_stages(db,schema):
    query='''select distinct stage_name FROM {db}.INFORMATION_SCHEMA.STAGES WHERE STAGE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and STAGE_SCHEMA='{schema}'
            order by 1'''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[0])
    return lst

def get_list_views(db,schema):
    query='''select distinct table_name FROM {db}.INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA NOT IN ('INFORMATION_SCHEMA') and TABLE_SCHEMA='{schema}'
            order by 1'''.format(db=db,schema=schema)
    res=st.session_state['sf_con'].execute(query)
    cmd1=res.fetchall()
    lst=[]
    for row in cmd1: 
        lst.append(row[0])
    return lst
def snowflake_connection(acct,user1,pwd1,wh,db):
    try:
        conn=sc.connect(account=acct,user=user1,password=pwd1,warehouse=wh,database=db)
        con_obj=conn.cursor()
        st.session_state['sf_con']=con_obj
        st.session_state['connect']=True      
    except Exception:
        st.session_state['connect']=False
   
if 'connect' not in st.session_state:

    st.markdown("""
    <h2 style='color:#2F4F4F;font-family:algerian;margin-bottom:1px;text-align: center;'>
    <img src='data:image/png;base64,' style='width:60px;height:70px'/>  
    Snowflake  Login  <img src='data:image/png;base64,' style='width:40px;height:40px'/>
    </h2><br>""", unsafe_allow_html=True)
    st.session_state['login']=st.radio('login',['Login Config File','Enter Credentials'],label_visibility='hidden',horizontal=True)
    
    _,col,_=st.columns([1,17,9])

    if st.session_state['login']=='Enter Credentials':
        with col:
            st.session_state['acct']=st.text_input('Account')
            st.session_state['user1']=st.text_input('Username')
            st.session_state['pwd1']=st.text_input('Password',type='password')
            st.session_state['wh']=st.text_input('Warehouse')
            st.session_state['db']=st.text_input('Database')
            st.markdown(f""" <style> div.stButton > button:first-child {{ background-color: #13aa52;   border: 1px solid #13aa52;   border-radius: 4px;   box-shadow: rgba(0, 0, 0, .1) 0 2px 4px 0;   box-sizing: border-box;   color: #fff;   cursor: pointer;   font-family: "Akzidenz Grotesk BQ Medium", -apple-system, BlinkMacSystemFont, sans-serif;   font-size: 16px;   font-weight: 400;   outline: none;   outline: 0;   padding: 10px 25px;   text-align: center;   transform: translateY(0);   transition: transform 150ms, box-shadow 150ms;   user-select: none;   -webkit-user-select: none;   touch-action: manipulation; }} <style> """ , unsafe_allow_html=True)
            st.button('Connect',on_click=snowflake_connection,args=[st.session_state['acct'],st.session_state['user1'],st.session_state['pwd1'],st.session_state['wh'],st.session_state['db']])
    else:
        with col:
            uploaded_file = st.file_uploader("file upload",label_visibility='hidden')
            if uploaded_file is not None:
                # To read file as bytes:
                bytes_data = uploaded_file.getvalue()

                # To convert to a string based IO:
                stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))

                # To read file as string:
                string_data = stringio.read()
                l1=string_data.split(',')
                l2=[]
                for i in l1:
                    l2.append(i.split('=')[1].replace("'",""))

                snowflake_connection(l2[0],l2[1],l2[2],l2[3],l2[4]) 
                st.experimental_rerun()

else:

    if st.session_state['connect']==True:
        
        st.markdown('''<h2 style='color:#008080;font-family: Rockwell;text-align: center;'>Grant Access To Objects... <img src="https://cdn-icons-png.flaticon.com/128/891/891381.png" style='width:70px;height:50px'/></h2>''',unsafe_allow_html=True)
        st.session_state['level']=st.selectbox('select level',['USER LEVEL','WAREHOUSE LEVEL','DATABASE LEVEL','SCHEMA LEVEL','OBJECT LEVEL'])
        if  st.session_state['level']=='USER LEVEL':
            st.session_state['role']=st.selectbox('select role',get_role_list())
            st.session_state['us_list']=st.selectbox('select USER name',get_user_list())
            st.button("Grant",on_click=give_access_user,args=[st.session_state['us_list'],st.session_state['role']])
        if  st.session_state['level']=='WAREHOUSE LEVEL':
            st.session_state['role']=st.selectbox('select role',get_role_list())
            st.session_state['wh_list']=st.selectbox('select warehouse name',get_wh_list())
            st.button("Grant",on_click=give_access_wh,args=[st.session_state['wh_list'],st.session_state['role']])
        if  st.session_state['level']=='DATABASE LEVEL':
            st.session_state['role']=st.selectbox('select role',get_role_list())
            st.session_state['db_list']=st.selectbox('select database name',get_db_list())
            st.button("Grant",on_click=give_query,args=[st.session_state['db_list'],st.session_state['role']])
        if  st.session_state['level']=='SCHEMA LEVEL':
            st.session_state['role']=st.selectbox('select role',get_role_list())
            st.session_state['db_list']=st.selectbox('select database name',get_db_list())
            st.session_state['schema_list']=st.selectbox('select schema name',get_schema_list(st.session_state['db_list']))
            st.button("Grant",on_click=give_access_schema,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list']])
        if  st.session_state['level']=='OBJECT LEVEL':
            st.session_state['role']=st.selectbox('select role',get_role_list())
            st.session_state['db_list']=st.selectbox('select database name',get_db_list())
            st.session_state['schema_list']=st.selectbox('select schema name',get_schema_list(st.session_state['db_list']))
            st.session_state['obj_type_list']=st.selectbox('select type of objects',get_avialble_type_schema(st.session_state['db_list'],st.session_state['schema_list']))
            if st.session_state['obj_type_list']=='TABLES':
                st.session_state['table_list']=st.selectbox('select table name',get_list_tables(st.session_state['db_list'],st.session_state['schema_list']))
                st.button("Grant",on_click=give_access_table,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list'],st.session_state['table_list']])
            if st.session_state['obj_type_list']=='PROCEDURES':
                st.session_state['procedure_list']=st.selectbox('select procedure name',get_list_procedures(st.session_state['db_list'],st.session_state['schema_list']))
                st.button("Grant",on_click=give_access_procedure,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list'],st.session_state['procedure_list']])
            if st.session_state['obj_type_list']=='FUNCTIONS':
                st.session_state['function_list']=st.selectbox('select functions name',get_list_functions(st.session_state['db_list'],st.session_state['schema_list']))
                st.button("Grant",on_click=give_access_functions,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list'],st.session_state['function_list']])
            if st.session_state['obj_type_list']=='STAGES':
                  st.session_state['stage_list']=st.selectbox('select stages name',get_list_stages(st.session_state['db_list'],st.session_state['schema_list']))
                  st.button("Grant",on_click=give_access_stages,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list'],st.session_state['stage_list']])
            if st.session_state['obj_type_list']=='VIEWS':
                st.session_state['view_list']=st.selectbox('select views name',get_list_views(st.session_state['db_list'],st.session_state['schema_list']))
                st.button("Grant",on_click=give_access_view,args=[st.session_state['db_list'],st.session_state['role'],st.session_state['schema_list'],st.session_state['view_list']])
    else:
        
        st.write('''<b><h2 style='font-family:Georgia;text-align:center;color:#b22222'>Invalid Credentials...🔐</h2></b>''',unsafe_allow_html=True)   
        st.session_state['login']=st.radio('login',['Login Config File','Enter Credentials'],label_visibility='hidden',horizontal=True)
    
        _,col,_=st.columns([1,17,9])

        if st.session_state['login']=='Enter Credentials':
            with col:
                st.session_state['acct']=st.text_input('Account')
                st.session_state['user1']=st.text_input('Username')
                st.session_state['pwd1']=st.text_input('Password',type='password')
                st.session_state['wh']=st.text_input('Warehouse')
                st.session_state['db']=st.text_input('Database')
                st.markdown(f""" <style> div.stButton > button:first-child {{ background-color: #13aa52;   border: 1px solid #13aa52;   border-radius: 4px;   box-shadow: rgba(0, 0, 0, .1) 0 2px 4px 0;   box-sizing: border-box;   color: #fff;   cursor: pointer;   font-family: "Akzidenz Grotesk BQ Medium", -apple-system, BlinkMacSystemFont, sans-serif;   font-size: 16px;   font-weight: 400;   outline: none;   outline: 0;   padding: 10px 25px;   text-align: center;   transform: translateY(0);   transition: transform 150ms, box-shadow 150ms;   user-select: none;   -webkit-user-select: none;   touch-action: manipulation; }} <style> """ , unsafe_allow_html=True)
                st.button('Connect',on_click=snowflake_connection,args=[st.session_state['acct'],st.session_state['user1'],st.session_state['pwd1'],st.session_state['wh'],st.session_state['db']])
        else:
            with col:
                uploaded_file = st.file_uploader("file upload",label_visibility='hidden')
                if uploaded_file is not None:
                    # To read file as bytes:
                    bytes_data = uploaded_file.getvalue()

                    # To convert to a string based IO:
                    stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))

                    # To read file as string:
                    string_data = stringio.read()
                    l1=string_data.split(',')
                    l2=[]
                    for i in l1:
                        l2.append(i.split('=')[1].replace("'",""))

                    snowflake_connection(l2[0],l2[1],l2[2],l2[3],l2[4])
                    st.experimental_rerun()